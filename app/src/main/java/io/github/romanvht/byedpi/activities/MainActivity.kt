...
class MainActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.navigationView.setNavigationItemSelectedListener { item ->
            when (item.itemId) {
                R.id.nav_home -> binding.drawerLayout.close()
                ...
                R.id.nav_about -> {
                    ...
                    MaterialAlertDialogBuilder(this)
                        ...
                        .setMessage(
                            getString(
                                R.string.about_masterdpi_message,
                                getString(R.string.telegram_profile),
    ...
    override fun onResume() {
        ...
        binding.wallpaperVideo.start()
    }

    private fun setupWallpaperVideo() {
        val uri = Uri.parse("android.resource://$packageName/${R.raw.alya}")
        binding.wallpaperVideo.setVideoURI(uri)
        binding.wallpaperVideo.setOnPreparedListener { mp ->
            mp.isLooping = true
            @Suppress("DEPRECATION")
            mp.setVolume(0f, 0f)
        }
        binding.wallpaperVideo.setOnErrorListener { _, _, _ -> true }
        binding.wallpaperVideo.start()
    }
            ...
            R.id.action_save_logs -> {
                val intent =
                    Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
                        ...
                        type = "text/plain"
                        putExtra(Intent.EXTRA_TITLE, "masterdpi.log")
                    }
